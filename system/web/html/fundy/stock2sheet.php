...
foreach ( $exchangesArray as $item ) {
	$SID = $item ['google_sheet_id'];
	
	$tablearray = sql_select_array ( "SELECT company,
		ticker_google AS 'ticker'
		FROM fundy._stock_" . $item ['symbol_google'] . "
		" );
	...
	for($i = 0; $i < count ( $tablearray ); $i ++) {
		$tarray [$i] = array (
				...
				'=GoogleFinance($B' . ($i + 3) . ',S$1)',
				'=GoogleFinance($B' . ($i + 3) . ',T$1)',
				'=GoogleFinance($B' . ($i + 3) . ',U$1)' 
		);
	}
